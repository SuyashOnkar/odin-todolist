...
(function indexPage() {
  ...
  function generateTasksandAddtoDom(project) {
    ...
    project.getTaskList.forEach((p) => {
      ...
      //ADD event listener to todos so I can click them to open the more menu
      task.addEventListener('click', () => {
        const taskDetailsDiv = document.createElement('div');
        taskDetailsDiv.classList.add('taskDetailsDiv');
        taskDetailsDiv.innerHTML = `<h1>${p.getName}</h1>`;
        taskDisplayDiv.appendChild(taskDetailsDiv);
      });
    });
  }
  ...
  taskInputDiv.innerHTML =
    '<form id="submitForm"><input class="textBox" required></input><div class="taskDateDiv"><input id="taskDate" type="date"></input></div><input type="submit"></input></form>';
